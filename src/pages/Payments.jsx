                ...
                {payments.map(p => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td data-label="Category"><span style={{ fontSize: 12, background: `${cat.color}22`, color: cat.color, padding: '2px 8px', borderRadius: 99 }}>{cat.label}</span></td>
                      <td data-label="Paid On" style={{ color: 'var(--text-secondary)' }}>{formatDate(p.paid_date)}</td>
                      <td data-label="Note" style={{ color: 'var(--text-muted)', fontSize: 13 }}>{p.note || '—'}</td>
                      <td data-label="Amount" style={{ fontWeight: 700, color: '#6ee7b7', fontFamily: 'Space Grotesk, sans-serif' }}>{formatCurrency(p.amount, currency)}</td>
                    </tr>
                  );
                })}
              </tbody>
